...
namespace ChatUI
{
    public class MainComponent : ViewModelBase
    {
        ...
        private Thread _ShareThread;

        private ShareService _server;
        ...
        /// </summary>
        private WebSocket WebSocketClient { get; set; }
        ...
        public NoParameterCommand AdminCommand { get; set; }
        ...
        private void InitShareService()
        {
            try
            {
                ...
                _ShareThread = new Thread(() => { _server.Run(); });
                _ShareThread.Start();
            }
            catch (Exception ex)
            {
        ...
        private void InitCommand()
        {
            try
            {
                ...
                AdminCommand = new NoParameterCommand(AdminCommandAction);
            }
            catch (Exception ex)
            {
        ...
        private void AdminCommandAction()
        {
            ...
            try
            {
                ...
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (dialog.FileName.Substring(0, 2).Equals("\\\\"))
                    {
                        ...
                        {
                            client.Credentials = new NetworkCredential("anonymous", "anonymous@example.com");
                            client.UploadFile(ftpServer, WebRequestMethods.Ftp.UploadFile,
                                tempFolder + "\\" + dialog.SafeFileName);
                        }
                        ...
                        WebSocketClient.Send(
                            $"[{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}] {UserName}({CurrentIP}) : {ftpServer}\n");
                    }
                    else
                    {
        ...
        private void ReadSetting()
        {
            try
            {
                ...
                if (File.Exists("UserSetting.ini"))
                {
                    ...
                    var convertSetting = JsonConvert.DeserializeObject<UserSetting>(setting);
                ...
                else
                {
                    ...
                    var defaultSetting = "{\"UserName\":\"" + localUserName + "\", \"ShowTime\":30}";
            ...
            try
            {
                ...
                if (receiveData.Contains("開始分享畫面"))
                {
                    ...
                    App.Current.Dispatcher.Invoke((Action)delegate // <--- HERE
                    {
                        ...
                        {
                            var data = AllUser.FirstOrDefault(x => x.UserIP == currentLivingUserIP);
                            if (data != null)
                            {
                                data.IsLive = Visibility.Visible;
                            }
                        }
                    });
                    return;
                ...
                if (receiveData.Contains("停止分享畫面"))
                {
                    ...
                    App.Current.Dispatcher.Invoke((Action)delegate // <--- HERE
                    {
                        if (AllUser.Any(x => x.UserIP == currentLivingUserIP))
                        {
                            AllUser.Where(x => x.UserIP == currentLivingUserIP).FirstOrDefault().IsLive =
                    ...
                    if (!alreadyJoin)
                    {
                        ...
                        if (allLloginMessage.Length > 2)
                        {
                            App.Current.Dispatcher.Invoke((Action)delegate // <--- HERE
                            {
                                ...
                                if (AllUser.All(x => x.UserIP != userIP))
                                {
                                    AllUser.Add(new UserInfo()
                                    {
                                        UserIP = userIP,
                ...
                {
                    ShowMessage("通知", receiveData, NotificationType.Success);
                    var allLoginMessage = receiveData.Split(' ');
                    if (allLoginMessage.Length > 2)
                    {
                        App.Current.Dispatcher.Invoke((Action)delegate
                        {
                            var userIP = allLoginMessage[1].Split(':')[0];
                            if (!AllUser.Any(x => x.UserIP == userIP)) return;
                        ...
                        case WindowState.Minimized:
                            ...
                            var th = new Thread(() =>
                            {
                                while (State != WindowState.Normal)
                                {
                                    ...
                                    {
                                        var wih = new WindowInteropHelper((Window)Window);
                                        FlashWindow(wih.Handle, true);
                                    });
                                    Thread.Sleep(1000);
                                }
        ...
        {
            return !receiveData.Contains("[img]");
        }
        ...
        public void SendMessage()
        {
            try
            {
                ...
                byte[] dataByteArray = Encoding.UTF8.GetBytes(
                    $"[{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}] {UserName}({CurrentIP}) : {InPut}\n");
